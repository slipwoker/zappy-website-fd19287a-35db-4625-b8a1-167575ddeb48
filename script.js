document.addEventListener('DOMContentLoaded', function () {
  ...
  const animatableElements = document.querySelectorAll(
    '[data-animate], .fade-in, .slide-up, .slide-in-left, .slide-in-right, .scale-in'
  );

  if (animatableElements.length > 0) {
    // Set initial state for elements that don't already have data-animate
    animatableElements.forEach(function (el, index) {
      if (!el.hasAttribute(ANIMATION_ATTR)) {
        el.setAttribute(ANIMATION_ATTR, 'fade-in');
      }

      // Stagger delay support
      const delay = el.dataset.animateDelay || (el.dataset.stagger ? index * 100 : 0);
      if (delay) {
    ...
    if ('IntersectionObserver' in window) {
      ...
      function isInViewport(el) {
        ...
        return (
          rect.top <= (window.innerHeight || document.documentElement.clientHeight) * (1 - ANIMATION_THRESHOLD) &&
          rect.bottom >= 0
        );
      }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "וילון גלאס Vilon Glass משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'fd19287a-35db-4625-b8a1-167575ddeb48',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'fd19287a-35db-4625-b8a1-167575ddeb48';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'fd19287a-35db-4625-b8a1-167575ddeb48';
